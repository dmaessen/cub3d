...
/*   Updated: 2024/02/19 15:26:16 by ahornstr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_map
{
	...
	int			map_x;
	int			map_y;
	double		side_dist_x; // length ray current pos
	double		side_dist_y;
	double		delta_dist_x; // length ray to next
	double		delta_dist_y;
	double		wall_dist;
	int			step_x; // direction to step in x or y-dir
	int			step_y;
 ...
 * then reads the file line by line with getnextline and sends each line 
 * to parse_map()
 * when the whole file has been read, its being closed
 * and then we proceed to is_walkable_path() (ensuring the player can at 
 * least make one step in the map)
 * then we parse_spaces() (to check if all the spaces in the map 
...
/**
 * loops through the map looking for the players position, when 
 * identified sends it
 * to check_walkable()
 * @param data main struct
 */
 ...
 * if it returns 1 then displays an error message that the map is 
 * not properly surrounded by walls.
 * @param data main struct
 */
void			parse_spaces(t_data *data);
/**
 * loops through the map until its on the given row, and then checks 
 * at the position pos to see if its not a 0/N/S/E/W at that position, 
 * if that would be the case then it returns 1 meaning the map is 
 * not properly surrounded by walls. else if all good returns 0.
 * @param map
 * @param row row in the map to check
 ...
 * then proceeds to check if the path has a .png or .jpg format 
 * else throws an error message
 * @param path path to texture
 */
void			check_image(char *path);
/**
 * first checks all the textures are present in the struct, else throws 
 * an error message mentionning the texture that is missing
 * if all four textures are there, checks if they are all different from 
 * each other, if not throws an error message about two textures 
 * being the same.
